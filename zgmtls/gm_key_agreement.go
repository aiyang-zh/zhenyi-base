...
func (ka *ecdheKeyAgreementGM) generateServerKeyExchange(config *Config, signCert, cipherCert *Certificate,
	...
	_ = cipherCert
	// GM/T 0024：ECDHE 使用 SM2 曲线（与 secp256r1 同参）；named curve 使用 TLS CurveID 23（secp256r1）
	curve := sm2.P256()
	ka.curveid = CurveP256

	privKey, x, y, err := elliptic.GenerateKey(curve, config.rand())
	if err != nil {
		return nil, err
	}
	ka.privateKey = privKey
	...
	if sigType != signatureSM2 {
		return nil, errors.New("tls: GM ECDHE requires SM2 signing certificate")
	}
...
func (ka *ecdheKeyAgreementGM) processClientKeyExchange(config *Config, cert *Certificate, ckx *clientKeyExchangeMsg, version uint16) ([]byte, error) {
	...
	if int(ckx.ciphertext[0]) != len(ckx.ciphertext)-1 {
		return nil, errClientKeyExchange
	}
...
func (ka *ecdheKeyAgreementGM) processServerKeyExchange(config *Config, clientHello *clientHelloMsg, serverHello *serverHelloMsg, cert *x509.Certificate, skx *serverKeyExchangeMsg) error {
	...
	ka.curveid = CurveID(uint16(skx.key[1])<<8 | uint16(skx.key[2]))
	...
	_, sigType, hashFunc, err := pickSignatureAlgorithm(cert.PublicKey, clientHello.supportedSignatureAlgorithms, supportedSignatureAlgorithms, ka.version)
	if err != nil {
		return err
	}

	sigLen := int(uint16(sig[0])<<8 | uint16(sig[1]))
	if sigLen+2 != len(sig) {
		return errServerKeyExchange
	}
	sig = sig[2:]
...
func (ka *eccKeyAgreementGM) processClientKeyExchange(config *Config, cert *Certificate, ckx *clientKeyExchangeMsg, version uint16) ([]byte, error) {
	...
	plainCipher, err := cipherUnmarshalSM2ToPlain(cipherDER)
	if err != nil {
		return nil, err
	}
	plain, err := decrypter.Decrypt(config.rand(), plainCipher, nil)
	if err != nil {
		return nil, err
	}
...
func (ka *eccKeyAgreementGM) processServerKeyExchange(config *Config, clientHello *clientHelloMsg, serverHello *serverHelloMsg, cert *x509.Certificate, skx *serverKeyExchangeMsg) error {
	...
	if !sm2.VerifyASN1(pubKey, digest, sig) {
...
func (ka *eccKeyAgreementGM) generateClientKeyExchange(config *Config, clientHello *clientHelloMsg, cert *x509.Certificate) ([]byte, *clientKeyExchangeMsg, error) {
	...
	encrypted, err := sm2.Encrypt(config.rand(), pubKey, preMasterSecret, sm2.NewPlainEncrypterOpts(sm2.MarshalUncompressed, sm2.C1C3C2))
	if err != nil {
		return nil, nil, err
	}
	// GMT0024 通信时密文采用 ASN.1 组织（与历史 tjfoc CipherMarshal 一致）
	encrypted, err = cipherMarshalSM2Plain(encrypted)
	if err != nil {
		return nil, nil, err
	}
