...
| `run_codeql_local.sh` | `make codeql-local` | **本地 CodeQL**（需 [CodeQL CLI](https://github.com/github/codeql-cli-binaries/releases) 与 `export CODEQL=…/codeql`）；**`database create`** 使用 **`.github/codeql/codeql-config.yml`**；**`analyze`** 不传额外查询以便 **query-filters** 生效。可选 **`CODEQL_LOCAL_FORMAT=sarif-latest`**。产物在 **`.codeql/`**（已 gitignore） |
