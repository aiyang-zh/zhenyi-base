...
**[官网](https://zhenyi-site.pages.dev/)** · [zhenyi-base GitHub](https://github.com/aiyang-zh/zhenyi-base) · [zhenyi 框架 GitHub](https://github.com/aiyang-zh/zhenyi)
...
> 基于 zhenyi-base 的完整 Actor 实时框架见：[`zhenyi`](https://github.com/aiyang-zh/zhenyi)
...
| **ztcp** | TCP 传输 | 高性能 TCP；**`ServerReactor`**（Linux epoll / macOS kqueue）+ 可选共享写 flush |
...
**模糊测试**：部分包提供 **`Fuzz*`** 用例，可在对应目录执行 **`go test -fuzz=Fuzz… -fuzztime=…`**，详见 [docs/API.md](docs/API.md#模糊测试可选)。
...
├── ztcp/          # TCP 传输（含 ServerReactor：Linux/macOS）
├── zws/           # WebSocket 传输
├── zkcp/          # KCP 传输
├── zreactor/      # reactor 循环（Linux epoll / macOS kqueue，供 ztcp.ServerReactor）
├── zserver/       # 轻量服务器
├── ziface/        # 核心接口定义
...
想快速搭建完整 Actor 实时业务框架？直接查看 **[zhenyi](https://github.com/aiyang-zh/zhenyi)**。
